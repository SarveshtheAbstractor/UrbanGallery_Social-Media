  ...
  if($_GET['usernamefg']){
   ...
   while($row = $result->fetch_assoc()){
      ...
      $rowuser = $resultsar->fetch_assoc();
         ...
         $profquery = "SELECT * FROM Profile WHERE userid=".$rowuser['id']." ORDER BY id DESC LIMIT 1";
      ...
      if($_GET['usernamefg']){
       ...
       $a = "<a class='toggleFollow btn btn-outline-primary btn-sm float-right' data-userId='".$row['isFollowing']."'><span><i class='fa fa-hand-peace-o mr-1'></i></span>Unfollow</a>";
         ...
         if($dataprof['image_path']!=""){
            $sad=  '<img class="media-object rounded-circle mr-3 " style="height: 100px; width: 100px;" src="upload/'.$dataprof['image_path'].'">';}
            else{
